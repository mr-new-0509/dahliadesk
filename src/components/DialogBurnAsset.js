...
import { Button, Dialog, DialogActions, DialogContent, DialogTitle, IconButton, Link, Stack, TextField, Typography } from '@mui/material';
...
export default function DialogBurnAsset({ dialogOpened, setDialogOpened, asset, setDesireReload, dialogTitle }) {
  ...
  const formik = useFormik({
    initialValues: {
      recipient: BURNER_VAULT_ADDRESS,
      amount: 1,
      note: ''
    },
      ...
      <DialogContent>
        <Stack spacing={2}>
          <TextField
            ...
            InputProps={{
              endAdornment: <Button variant="outlined" component={Link} target="_blank" href={`${baseUrlOfExplorer}/address/${BURNER_VAULT_ADDRESS}`}>
                View
              </Button>
            }}
            disabled
